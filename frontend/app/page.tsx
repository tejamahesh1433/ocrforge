...
export default function Home() {
  ...
  function selectFile(selected: File | null) {
    ...
    if (!["png", "jpg", "jpeg", "pdf"].includes(extension || "")) {
      setError("Supported formats: PNG, JPG, JPEG and PDF");
      return;
    }
  ...
  function download(content: string, extension: string, type: string) {
    ...
    anchor.href = url;
    anchor.download = `${baseName}-ocr.${extension}`;
    anchor.click();
  ...
  function downloadJson() {
    ...
    download(payload, "json", "application/json");
  }
        ...
        <div className="grid gap-6 lg:grid-cols-2">
            ...
            {!file ? (
              <div className="p-6">
                <label
                  ...
                  onDrop={handleDrop}
                  className={`flex min-h-[500px] cursor-pointer items-center justify-center rounded-xl border-2 border-dashed p-8 text-center transition ${
                    dragging
          ...
          <section className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 px-6 py-4">
              <h2 className="font-semibold">OCR Result</h2>
            ...
            {result && (
              <div className="flex border-b border-slate-800 px-4 pt-3">
                {(["rendered", "markdown", "text"] as ViewMode[]).map(
                  (mode) => (
                    <button
                      key={mode}
            ...
            <div className="h-[650px] overflow-auto bg-slate-950 p-6">
              {loading ? (
                  ...
                  {viewMode === "rendered" && (
                    <article className="prose prose-invert max-w-none whitespace-pre-wrap">
                      <ReactMarkdown remarkPlugins={[remarkGfm]}>
                        {result.markdown.replace(/<\/?PAGE>/gi, "")}
                      </ReactMarkdown>
                    </article>
                  )}
                ...
                <div className="flex h-full items-center justify-center text-center text-slate-600">
                  <div>
                    <p className="text-lg">No OCR result yet</p>
                    <p className="mt-2 text-sm">
                      Upload a document and run OCR.
                    </p>
                  </div>
                </div>
              )}
            ...
            {result && (
              <div className="border-t border-slate-800 px-6 py-4 text-xs text-slate-500">
                <div className="flex flex-wrap gap-x-6 gap-y-2">
                  ...
                  <span>
                    {result.processing_time_seconds.toFixed(1)} sec
                  </span>
                  ...
                  <span>
                    GPU {result.gpu.vram_peak_allocated_gb.toFixed(2)} GB peak
                  </span>
